...
class TestSaveLoadWithEvents:
    ...
    def test_save_falls_back_when_copy2_raises_permission_error(self, temp_save_dir, tmp_path):
        """Test save still succeeds when metadata-preserving DB copy is rejected."""
        db_path = tmp_path / "events.db"
        game_map = create_test_map()
        month_stamp = create_month_stamp(Year(100), Month.JANUARY)
        ...
        world.event_manager.add_event(make_event(100, 1, "Fallback copy event"))

        sim = Simulator(world)
        save_path = temp_save_dir / "copy2_fallback.json"
        events_db_path = save_path.with_suffix("").with_name(save_path.stem + "_events.db")

        with patch(
            "shutil.copy2",
            side_effect=PermissionError(13, "Permission denied", str(events_db_path)),
        ):
            success, _ = save_game(world, sim, [], save_path)
    ...
    def test_save_succeeds_when_copyfile_opening_target_is_rejected(self, temp_save_dir, tmp_path):
        """Test save still succeeds when direct file copy is rejected on Android-like filesystems."""
        db_path = tmp_path / "events.db"
        game_map = create_test_map()
        month_stamp = create_month_stamp(Year(100), Month.JANUARY)
        ...
        world.event_manager.add_event(make_event(100, 1, "Backup export event"))

        sim = Simulator(world)
        save_path = temp_save_dir / "copyfile_permission_denied.json"
        events_db_path = save_path.with_suffix("").with_name(save_path.stem + "_events.db")
        ...
        assert save_path.exists()
        assert events_db_path.exists()
        exported_storage = EventStorage(events_db_path)
        assert exported_storage.count() == 1
        exported_storage.close()

        world.event_manager.close()

    def test_save_replaces_read_only_existing_events_db(self, temp_save_dir, tmp_path):
        """Test save can replace a stale read-only events DB file."""
        db_path = tmp_path / "events.db"
        game_map = create_test_map()
        month_stamp = create_month_stamp(Year(100), Month.JANUARY)
        ...
        world.event_manager.add_event(make_event(100, 1, "Read-only target event"))

        sim = Simulator(world)
        save_path = temp_save_dir / "readonly_target.json"
        events_db_path = save_path.with_suffix("").with_name(save_path.stem + "_events.db")
        events_db_path.write_bytes(b"stale-db")
        os.chmod(events_db_path, stat.S_IREAD)

        try:
            success, _ = save_game(world, sim, [], save_path)
        finally:
            if events_db_path.exists():
                os.chmod(events_db_path, stat.S_IWRITE | stat.S_IREAD)
