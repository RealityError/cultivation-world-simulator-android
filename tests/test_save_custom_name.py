...
class TestSaveGameWithCustomName:
    ...
    def test_save_failure_returns_error_message(self, temp_save_dir):
        """Test that save_game returns the underlying error message on failure."""
        game_map = create_test_map()
        world = World(map=game_map, month_stamp=create_month_stamp(Year(100), Month.JANUARY))
        sim = Simulator(world)

        with patch("builtins.open", side_effect=OSError("disk full")):
            success, error_message = save_game(
                world, sim, [],
                save_path=temp_save_dir / "broken.json",
                custom_name=None
            )

        assert success is False
        assert error_message == "disk full"

    def test_save_succeeds_when_event_db_metadata_copy_would_fail(self, temp_save_dir):
        """Test that save_game does not depend on shutil.copy2 metadata propagation."""
        game_map = create_test_map()
        world = World(map=game_map, month_stamp=create_month_stamp(Year(100), Month.JANUARY))
        base_events_db = temp_save_dir / "base_events.db"
        world.event_manager = EventManager.create_with_db(base_events_db)
        sim = Simulator(world)

        with patch("shutil.copy2", side_effect=PermissionError("metadata copy denied")):
            success, saved_name = save_game(
                world,
                sim,
                [],
                save_path=temp_save_dir / "copyfile_fallback.json",
                custom_name=None,
            )

        assert success is True
        assert saved_name == "copyfile_fallback.json"
        assert (temp_save_dir / "copyfile_fallback.json").exists()
        assert get_events_db_path(temp_save_dir / "copyfile_fallback.json").exists()
...
class TestSaveApiWithCustomName:
    ...
    def test_api_save_returns_underlying_error_detail(self, temp_save_dir):
        """Test API save endpoint returns the underlying save error detail."""
        from fastapi.testclient import TestClient
        from src.server import main
        from src.utils.config import CONFIG
        ...
        with patch.object(CONFIG.paths, "saves", temp_save_dir), \
             patch("src.server.main.save_game", return_value=(False, "disk full")):
            client = TestClient(main.app)
            response = client.post("/api/game/save", json={})

        assert response.status_code == 500
        assert response.json()["detail"] == "保存失败: disk full"
