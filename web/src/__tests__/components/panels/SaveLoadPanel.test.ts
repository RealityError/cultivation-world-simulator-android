...
vi.mock('naive-ui', () => ({
  NModal: {
    name: 'NModal',
    template: '<div class="n-modal" v-if="show" :style="style"><slot /><slot name="footer" /></div>',
    props: ['show', 'title', 'preset', 'maskClosable', 'closable', 'style'],
  },
  NInput: {
    name: 'NInput',
  ...
  messages: {
    'en-US': {
      save_load: {
        ...
        save_error_detail: 'Save error details',
        copy_error: 'Copy',
...
describe('SaveLoadPanel', () => {
  ...
  describe('Save Mode', () => {
    ...
    it('should switch to stacked mobile save layout when mobile mode is active', async () => {
      ...
      expect(wrapper.classes()).toContain('save-load-panel--mobile')
      expect(wrapper.find('.save-actions').classes()).toContain('save-actions--mobile')
    })
  })
  ...
  describe('Error Handling', () => {
    ...
    it('should handle saveGame error gracefully', async () => {
      vi.mocked(systemApi.fetchSaves).mockResolvedValue({ saves: [] })
      vi.mocked(systemApi.saveGame).mockRejectedValue(new Error('磁盘已满'))
      ...
      expect(mockMessageError).toHaveBeenCalledWith('磁盘已满')
    })

    it('should render named save error inside modal instead of global toast', async () => {
      vi.mocked(systemApi.fetchSaves).mockResolvedValue({ saves: [] })
      vi.mocked(systemApi.saveGame).mockRejectedValue(new Error('磁盘已满'))
      ...
      await wrapper.find('.new-save-card').trigger('click')
      await flushPromises()
      await wrapper.find('.n-input').setValue('test_save')
      await flushPromises()
      await wrapper.findAll('.modal-footer .n-button')[1].trigger('click')
      await flushPromises()

      expect(wrapper.find('.save-submit-error').exists()).toBe(true)
      expect((wrapper.find('.save-submit-error-text').element as HTMLTextAreaElement).value).toBe('磁盘已满')
      expect(mockMessageError).not.toHaveBeenCalled()
    })

    it('should copy named save error text from modal', async () => {
      ...
      await wrapper.findAll('.modal-footer .n-button')[1].trigger('click')
      await flushPromises()
      await wrapper.find('.save-submit-error-copy').trigger('click')
      await flushPromises()

      expect(mockClipboardWriteText).toHaveBeenCalledWith('磁盘已满')
    })

    it('should clear named save error after input changes', async () => {
      vi.mocked(systemApi.fetchSaves).mockResolvedValue({ saves: [] })
      vi.mocked(systemApi.saveGame).mockRejectedValue(new Error('磁盘已满'))
      ...
      await wrapper.find('.n-input').setValue('test_save_2')
      await flushPromises()

      expect(wrapper.find('.save-submit-error').exists()).toBe(false)
    })
  })
