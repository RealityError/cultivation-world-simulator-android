...
def _clear_stale_events_db(path: Path) -> None:
    if not path.exists():
        return
...
def _snapshot_events_db(current_db_path: Path, target_db_path: Path, storage) -> None:
    target_db_path.parent.mkdir(parents=True, exist_ok=True)
    temp_db_path = target_db_path.with_name(f"{target_db_path.name}.tmp")
    _clear_stale_events_db(temp_db_path)

    current_conn = getattr(storage, "_conn", None) if storage is not None else None
    current_lock = getattr(storage, "_db_lock", None) if storage is not None else None

    try:
        if current_conn is not None and current_lock is not None:
            with current_lock:
                current_conn.commit()
                snapshot_conn = sqlite3.connect(str(temp_db_path))
                try:
                    current_conn.backup(snapshot_conn)
                    snapshot_conn.commit()
                finally:
                    snapshot_conn.close()
        else:
            shutil.copyfile(current_db_path, temp_db_path)

        if target_db_path.exists():
            _clear_stale_events_db(target_db_path)
...
def save_game(
    ...
    try:
        ...
        if hasattr(world.event_manager, "_storage") and world.event_manager._storage:
             ...
             if current_db_path != events_db_path:
                 ...
                 if current_db_path.exists():
                    # 安卓真机上可能残留只读的旧 events.db，保存前先清理目标，再导出 SQLite 快照。
                    _snapshot_events_db(current_db_path, events_db_path, storage)
        ...
        print(f"Failed to save game: {error_message}")
        import traceback
        traceback.print_exc()
