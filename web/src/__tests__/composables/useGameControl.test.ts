...
describe('useGameControl', () => {
  ...
  it('closes mobile event panel before opening system menu', () => {
    const { component } = createTestComponent()
    const wrapper = mount(component)

    uiStore.mobileEventPanelOpen = true

    const handled = wrapper.vm.handleBackAction()

    expect(handled).toBe(true)
    expect(uiStore.mobileEventPanelOpen).toBe(false)
    wrapper.unmount()
  })
